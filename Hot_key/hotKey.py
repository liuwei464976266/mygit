...
def loadConfig():
    ...
    cf.read('config.cfg')
    LOGANALYSISUTIL_DIR_PATH = cf.get('config','LOGANALYSISUTIL_DIR_PATH')
    PIC_PATH = cf.get('config', 'PIC_PATH')
    SPECIAL_PIC_PATH = cf.get('config', 'SPECIAL_PIC_PATH')
    USERNAME_HANLE_DIC = cf.get('config', 'USERNAME_HANLE_DIC')
    return LOGANALYSISUTIL_DIR_PATH,PIC_PATH,SPECIAL_PIC_PATH,USERNAME_HANLE_DIC
LOGANALYSISUTIL_DIR_PATH,PIC_PATH,SPECIAL_PIC_PATH,USERNAME_HANLE_DIC = loadConfig()
sys.path.append(LOGANALYSISUTIL_DIR_PATH)
import win32con
import ctypes
    ...
    for dirpath,dirnames,filenames in os.walk(path):
        for i in sorted(filenames):
            if i.endswith('.png'):
                ...
                template = cv2.imread(pic_path, cv2.IMREAD_UNCHANGED)
                template = cv2.cvtColor(template, cv2.COLOR_BGRA2GRAY)
                picPath_list.append((i,template))
        break
    return picPath_list
...
def getPoint(handle,pic_info):#获取图片坐标
    path = pic_info[0]
    pic_data = pic_info[1]
    image = capture(handle)
    gray = cv2.cvtColor(image, cv2.COLOR_BGRA2GRAY)
    result = cv2.matchTemplate(gray, pic_data, cv2.TM_CCORR_NORMED)
    # 获取结果中最大值和最小值以及他们的坐标
...
def play(handle):
    ...
    while runningEnble:
        ...
        for pic in pic_list:
            ...
            if point != (-1, -1):
                leftClick(handle,point[0],point[1])
                time.sleep(1)
                if pic[0] == r'b_jz.png':
...
def jump(func, hotkey):
    ...
    lock = threading.Lock()
    runningEnble = True
    pic_list = get_picPath(PIC_PATH)
    special_pic_list = get_picPath(SPECIAL_PIC_PATH)
