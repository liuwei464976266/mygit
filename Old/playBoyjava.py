...
def checkNgold(response,betScore,lastNgold):#核对Ngold
    ...
    if response['et']['type'] == 3:  # 判断是否是红利场
        ...
        # if points.count(13) >= 3 or response['et']['data']['isFree']:
            # if response['et']['data']['residue'] < 10:
                # print("免费场中免费场有误")
                # exit(-4)
        ...
        Rpoints = response['et']['data']
        points = Rpoints['points']
        if points.count(13) >= 2 or response['et']['data']['isFree']:
            print("免费场中免费场有误")
            exit(-4)
        AdPoints = Rpoints['adPoints']
        LinePoints = Rpoints['linePoints']
...
def checkAdpoints(response):#核对points
    ...
    return True
def normalPlay(tokenid,gametype,betScore,playtimes,winRate):#正常玩模式
    ...
    try:
        ...
        response = response.json()
        print(f"第{playtimes}次游戏,胜率为{winRate}",response)
...
def getToken(username,password):
    ...
    data = json.dumps(data)
    url = "http://192.168.10.212:8001/callToken"
...
def main():#主函数 程序入口
    ...
    # pay_response = requests.get(url=f"http://192.168.10.212:8001/pay/{username}/50000000")
    while True:
        response = normalPlay(tokenid=tokenid, gametype=gameType,betScore = betScore,playtimes = playtimes,winRate = winRate)
        if response:
            playtimes += 1
            # if response['et']['type'] == 1 and (response['et']['data']['points'].count(13) >= 3 or response['et']['data']['isFree']):
            #     print("免费场中免费场有误")
            #     exit(-4)
            if response['et']['type'] == 4 and (response['et']['data']['points'].count(13) >= 3 or response['et']['data']['isFree']):
                print("免费场中免费场有误")
                exit(-4)
            ...
            if 'lastNgold' not in dir():  # 如果是第一次运行脚本
                ...
                if response['et']['data'].get('isFree', False) and response['et']['type'] == 0:
                ...
                #     break
            if payedGold != 0:
                winRate = (lastNgold - startGold) / payedGold
