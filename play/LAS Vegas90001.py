...
SLOTURL = 'http://192.168.10.25:9000/getSlotData'
REGISTERURL = 'http://192.168.10.25:9000/registerUser'
PLAY_BOY_AWARD = {
    (11, 1): 0,
    (11, 2): 1,
    (1, 3): 2,
...
def checkNgold(response, betScore, lastNgold):  # 核对Ngold
    ...
    if current_gold != response['et']['nGold']:
        print(f'游戏模式==={pp},=====筹码错误', paygold, current_gold, response['et']['nGold'])
        return False
    else:
            ...
            if response['et']['type'] == 0 and response['et']['data']['isFree'] == False:  # 纯普通场
                ...
                if response['et']['type'] == 1:
                    if bonusGold == 0 or myfreegold != 0:
                        ...
                        return
                    if response['et']['data']['isFree'] == True and response['et']['data']['specialRole'] == 1:
                        fsTimes = 25
                        playedTimes = 0
                        num = 0
                    ...
                    while playedTimes <= 50:
                        ...
                        if response:
                            ...
                            if 'lastnGold' not in dir():  # 如果是第一次运行脚本
                                ...
                                if lastachievement == False:
                                    return
                            if response['et']['data']['residue'] != playedTimes:
